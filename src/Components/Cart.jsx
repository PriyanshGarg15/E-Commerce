        ...
        <Layout>
            <div className="md:my-16 mx-auto md:w-7/12 bg-white shadow-lg border rounded-md p-8">
                ...
                <div className="space-y-12">
                    {
                        products.map((item,index)=>(
                            <div key={index} className="flex gap-4">
                                <img src={item.image} className="w-[100px] border border-3 border-white shadow" alt="" />
                                <div>
                                    <h1 className="font-semibold capitalize text-lg">{item.title}</h1>
                                    <div className="flex flex-col gap-4">
                                        <div className="space-x-3">
                                            <label htmlFor="" className="text-lg font-semibold">{(item.price)-(item.price*item.discount)/100}</label>
                                            <del>₹{item.price}</del>
                                            <label htmlFor="" className="text-gray-500">₹{item.discount}% Discount</label>
                                        </div>
                                        <button className="w-fit bg-rose-600 text-white px-4 py-2 rounded">
                                            <i className="ri-delete-bin-5-fill mr-2"></i>
                                            Remove
                                        </button>
                                    </div>
                                </div>
                ...
                <div className="flex justify-between items-center">
                    ...
                    <button className="w-fit bg-green-500 text-white px-12 py-2 rounded mt-4 hover:bg-rose-600">
